...
| **Self-encounters across tenants** | Unfiled, newly observed | The same physical vessel reported by two VMS tenants gets distinct `vessel_id`/`ssvid` and can "encounter itself" (~3.9% of encounters share a shipname across sides; ~96% of those cross-tenant). Pre-existing, not dit-induced. Detail + how to exclude: [`workflows/encounters/README.md`](../workflows/encounters/README.md). |
...
- **Wall-clock is not compute time.** The full-year A/B *looked* like ~16 hours; real Dataflow time was **~76 minutes** (create 53.4 / 61.1 min, merge 13.6 / 14.5 min, bindings concurrent). The remainder was the submitting laptop asleep — 3h37m between create finishing and merge starting, then ~11h before the final log line. Because the workflow submits step 2 only after step 1 returns, a sleeping laptop stalls the *chain*, not the jobs. **Read durations from `gcloud dataflow jobs describe` (`createTime` → `currentStateTime`), not from submitter log timestamps.**
- **A long-running submitter can outlive the operator's ADC token.** Afterwards `bq` failed with `Reauthentication failed. cannot prompt during non-interactive execution` and results were unreadable until re-auth. The Dataflow jobs were fine — own service-account credentials — so only the laptop-side submitter and follow-up queries are exposed. Precursor symptom, obvious only in hindsight: transient `TransportError: Unable to find the server at oauth2.googleapis.com` retries mid-run.
- **Sizing.** A full-year, all-vessel VMS `1_bf` A/B is **~1.3 hours of Dataflow** — cheap enough to be unremarkable. What does *not* scale is the daily-slice modes: `2_bfd` over a year is ~365 slices x 2 steps per binding and per-job startup dominates, so validate the incremental path on a short window regardless.
