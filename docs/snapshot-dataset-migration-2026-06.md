...
**Status (2026-06-09)**: **migration COMPLETE.** Policy landed PR #54; M1 landed PR #56 (`dit.bq.snapshot_into_experiment(...)` helper); M2 landed PR #57 (`outage_recovery.py`); M3 landed PR #58 (`identity_match_key.py` + pipe-segment cross-org `--snapshot-dest-project` tactical fix); M4 landed PR #62 (`ais.py` per-table `--source-{messages,segment-info,segs-activity}-fqn` override flags); **M5 landed (this commit) — `workflows/port_visits/cross_version_ais.py` migrated**, replacing the per-experiment `dit_exp_<exp>_{internal,published}` dataset shape with M1's `snapshot_into_experiment(role="cross_version")` and routing per-binding ais.py invocations via M4's per-table FQN flags. The transitional legacy-dataset protection rule has been removed from `CLAUDE.md` § Working agreements (no workflow creates `dit_exp_*` datasets anymore). One remaining adjacent item (cross-org Guard) is tracked separately. The synthetic-source-mutation primitive (issue #59) is tracked separately in [`source-mutation-primitive-2026-06.md`](source-mutation-primitive-2026-06.md); its progress is orthogonal to this migration and isn't reflected here.
...
| M5 | Convert `workflows/port_visits/cross_version_ais.py` | B | +61 LOC net (the frozen `_CrossVersionSnapshotFQNs` dataclass + explicit per-source `snapshot_into_experiment` calls + expanded migration-context comments offset the dataset-creation drops; the no-`create_dataset` goal is the load-bearing metric, mirroring M3's outcome) | 10 new unit tests pin the dataclass shape + `_snapshot_source` invocation pattern + `_ais_args_for_binding` emitting the M4 flags + load-bearing user-extras drops; cloud smoke deferred (the user-extras-drop tests are the structural guarantee that the cross-version pin is preserved) | **Landed 2026-06-09** |
