...
**M5a — cleanup control plane (`cancel_run` + `dit cache-cancel` + `make dit-cancel`).** Standalone; no port-visits dependency. `dit.cache.cancel_run(run_id)` is currently a `NotImplementedError` stub (`cache.py:544`); `dit.cli` is a `click` group, so `dit cache-cancel` is one new `@main.command`.
