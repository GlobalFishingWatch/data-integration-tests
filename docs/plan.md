...
4.5. **LLM PR-relevance pre-filter** (negative-signal-only). GitHub Models step in each pipeline repo's PR-trigger workflow trims docstring-only / formatting-only / test-only PRs that path filters can't distinguish from real changes. Two-phase rollout (log-only audit → enforce only after zero false negatives over ≥50 PRs). Fail-open at every step; humans escalate via `dit:run` label, never via the LLM. Design sketch: [`docs/llm-pr-gating.md`](llm-pr-gating.md).
