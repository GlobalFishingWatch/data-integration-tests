...
- **Live end-to-end validation of auto-build — done (2026-05-28).** A real `make dit-cloud` against the unreviewed pipe-gaps tree exercised the full path: snapshot → push → kaniko worker-image build (`gcr.io/world-fishing-827/dit/pipe-gaps:dit-<sha>`) → Dataflow workers pulled it → mode-equivalence (0 diff). The **dirty-tree common case is confirmed working.** It also surfaced two real `make dit-cloud REF=<ref>` cloud-path bugs (now fixed — CHANGELOG 2026-05-28): (A) `is_unreviewed`'s `git fetch origin main` fails in ditbox (SSH origin, no key) → every cloud run treated unreviewed; fixed by resolving `unreviewed` laptop-side and threading it as `_UNREVIEWED`/`DIT_UNREVIEWED`. (B) the auto-built worker image was built from the working tree, not the `REF` → submitter/worker mismatch; fixed by building from `git archive <commit>`. **Re-validated 2026-05-28** with the fixes in place: a `make dit-cloud REF=origin/main` run was correctly classified *reviewed* (laptop-resolved `DIT_UNREVIEWED=false` honored cloud-side — Bug A) and used the published `v0.9.6` image instead of mis-building the dirty tree (Bug B) — mode-equivalence then diverged **350 / 317 / 347** differing rows (the `v0.9.6` non-determinism reproduced) against the fix tree's **0 / 0 / 0**. That's the genuine main-vs-fix comparison the first attempt couldn't produce; both REF-path bugs confirmed fixed live.
- **`is_unreviewed` per-run `git fetch origin main`** — adds ~1s + a network dependency to every clean/override run on the laptop, and is unreachable in ditbox (resolved by the laptop-side `_UNREVIEWED` thread-through, 2026-05-28). If the laptop fetch becomes a bottleneck (e.g. many parallel cross-version bindings), consider a short-lived cache of the `origin/main` tip per process.
