...
- **Snapshot auto-push: assume origin is public.** `make dit-cloud` against a dirty pipeline checkout auto-creates a snapshot commit and pushes it to the pipeline repo's `origin`. Any pipeline repo could be (or become) a public GitHub repo. The snapshot mechanism is built with that assumption: only **tracked** file modifications + deletions are captured (via `git add -u` against a temp index seeded from HEAD); untracked files — including `git add`-ed-but-uncommitted new files — are **never** in the snapshot. This is the deliberate safety default; the convenience alternative (`git add -A`) was explicitly rejected because it would auto-push any rogue `.env`/`sa.json`/dataset/log lying in the working tree. When designing related features:
