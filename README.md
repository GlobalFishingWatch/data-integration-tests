...
- **Run cleanup control plane** (`dit cache-cancel RUN_ID` / `make dit-cancel RUN_ID=<id>`, `dit.cache.cancel_run`) — cancel all sibling modes of a run together: discovers the run's Dataflow jobs **by the `dit_run_id` label** both workflows stamp (the stored `dataflow_job_ids` are always empty), cancels the still-running ones, deletes the recorded output tables (**table-level only** — dataset-shaped or malformed values are skipped), and marks the rows `cancelled`. Idempotent. Landed M5a, 2026-05-29. The table-level guard is also the structural protection that keeps **all dit BQ artifacts in the canonical `world-fishing-827.tech_great_expectations` dataset**: dit never creates new datasets (the Cloud Build SA `automated-testing@` lacks `bigquery.datasets.create` by design), and the cancel path categorically refuses to escalate a delete to dataset scope. Three workflows currently still create per-experiment `dit_exp_*` datasets; migration to the canonical-dataset shape is in flight (see `CLAUDE.md` Plan changelog 2026-06-08).
