...
- **Three pipeline workflows today**: `workflows/pipe_gaps/mode_equivalence.py` (detect-gaps; in-process Beam; SCD-2 compare), `workflows/port_visits/ais.py` (port-visits; Beam-via-container; truncate compare on `visit_id`), `workflows/pipe_events/fishing.py` (fishing events; **BQ-SQL-via-container**; truncate compare on `event_id`). Each is a faithful port of that pipeline's bash/CLI mode-equivalence test plus the automated cross-mode comparison. See [`workflows/README.md`](workflows/README.md).
