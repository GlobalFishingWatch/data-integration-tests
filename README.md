...
2. **Run cache M6 — SIGTERM trap inside `dit run`** so Cloud Build cancellations cleanly tear down their own Dataflow + BQ artefacts via `cancel_run` (the natural follow-on now M5a has landed).
