...
1. **Per-pipeline PR triggers.** Wire `pipe-gaps` (first; it's the most-verified workflow today) → `anchorages_pipeline` → `pipe-events` to fire dit on PRs. Trigger config lives in each pipeline repo; references `cloudbuild-dit.yaml` from the dit checkout. Path-filter on pipeline-relevant files (`src/**`, `transforms/**`) to skip docs / config-only PRs. `dit:run` label as the escape-hatch override.
2. **PR comment integration.** Make dit verdicts surface on PRs the way unit tests do: a small results table in a comment or check-run. Recommended path: Cloud Build → Pub/Sub → a Cloud Function that parses `table-check`'s structured JSON and posts a markdown table via the GitHub API (Cloud Build native check-runs are status-only; an Action is over-engineered for a one-shot run).
3. **Track 5** — pipe-gaps repo shim, opportunistically (replace `pipe-gaps/tests/integration/mode_equivalence.py` with a thin re-export from `dit/workflows/pipe_gaps/...`).
4. VMS port-visits workflow, then AIS-full (the latter is what motivates Cloud Run jobs hardest once concurrent runs scale up).
5. PIPELINE-1465 cross-version full validation against newly-built pipe-anchorages images (parallel to the pipe-gaps validation just completed).
6. The first dit release tag (`v0.1.0`) once per-pipeline PR triggers land — that's the natural "framework is usable by outsiders" milestone.
