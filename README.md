...
1. **Run cache M5 — port-visits cache + cleanup control plane** (refined 2026-05-29; see [`docs/run-cache-impl.md`](docs/run-cache-impl.md) § Milestone 5). **M5a**: implement `cancel_run` + `dit cache-cancel` + `make dit-cancel` — discover Dataflow jobs by `dit_run_id` label (stored `dataflow_job_ids` are empty), `bq rm` the output tables, UPDATE status. **M5b**: wire port-visits caching — now just supply its own `canonical_params_dict` + `CacheKey` and call the shared `dit.workflow.run_with_cache` (the #28 harness extraction did the reusable half), flipping `ais.py`'s `resolve_digest=True`.
2. **Run cache M6 — SIGTERM trap inside `dit run`** so Cloud Build cancellations cleanly tear down their own Dataflow + BQ artefacts via `cancel_run` (follows M5a).
