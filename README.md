...
**Exception: `workflows/encounters/vms.py` defaults to PROD VMS sources.** There is no VMS equivalent of the AIS staging cohort, so this one workflow reads prod: `gfw-int-vms-v3.pipe_vms_v3_internal.{messages_positions,segs_activity,spatial_measures_clustered_vms_v20251209}` plus `global-fishing-watch.pipe_vms_v4_published.segment_info` — three projects across two GCP organisations, all US-located. The cost guard is the **date window, not the cohort**: defaults are a 7-day window (`2026-06-01`..`2026-06-07`) rather than the year-wide window the AIS workflows can afford against ~503 GB / 1.13 B rows. Measured on the query shape the pipeline actually issues, the default VMS run scans **538 MB** — *less than half* the **1,265 MB** `encounters/ais.py` scans over its own default window against the staging cohort, because MONTH partition pruning plus a narrow window beats a small-but-unpruned table. So this exception concedes the cohort, not the "never hit prod volume by default" rule. Prefer `--ssvid-filter` over widening the window. Justification for the exception is empirical: interpolation-boundary behaviour is effectively untestable on the AIS cohort (gaps of exactly 3600s occur once per ~269k) and richly testable on VMS (5.48% of all gaps, more common than gaps *longer* than an hour). Note the Dataflow worker SA has no direct IAM binding on either external project — see the workflow docstring.
